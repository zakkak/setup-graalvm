...
| `version` | `''` | `X.Y.Z` (e.g., `22.3.0`) for a specific [GraalVM release][releases] up to `22.3.2`<br>`mandrel-X.Y.Z` (e.g., `mandrel-21.3.0.0-Final`) for a specific [Mandrel release][mandrel-releases],<br>`mandrel-X.Y` (e.g. `mandrel-23.0`) for the latest stable Mandrel release on that release train,<br>`mandrel-latest` for [latest Mandrel stable release][mandrel-stable]. |
