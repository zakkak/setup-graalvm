...
| `version` | `''` | `X.Y.Z` (e.g., `22.3.0`) for a specific [GraalVM release][releases] up to `22.3.2`<br>`mandrel-X.Y.Z` (e.g., `mandrel-21.3.0.0-Final`),  or `X.Y.Z.W-Final` (e.g. `23.1.0.0-Final`) in combination with setting `distribution` to `mandrel` starting with `23.0.0`, for a specific [Mandrel release][mandrel-releases],<br>`mandrel-latest`, or `latest` in combination with setting `distribution` to `mandrel`, for [latest Mandrel stable release][mandrel-stable]. |
